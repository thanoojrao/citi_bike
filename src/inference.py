...
def load_batch_of_features_from_store(
    ...
    features = transform_ts_data_info_features(
        ts_data, window_size=4 * 28, step_size=1
    )
...
def fetch_predictions(hours):
    current_hour = (pd.Timestamp.now(tz="Etc/UTC") - timedelta(hours=hours)).floor("6H")
    print(f'current time: {pd.Timestamp.now(tz="Etc/UTC")}')
    print(current_hour)
    fs = get_feature_store()
    fg = fs.get_feature_group(name=config.FEATURE_GROUP_MODEL_PREDICTION, version=1)
...
def fetch_days_data(days):
    ...
    fetch_data_from = current_date -timedelta(weeks=12) - timedelta(days=(days))
    fetch_data_to = current_date - timedelta(weeks=12)
